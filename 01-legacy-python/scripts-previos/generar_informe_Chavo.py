"""Genera el informe de liderazgo para Chavo usando MODELO_DE_INFORME_1.docx como base.
Lee las respuestas directamente de 'Planilla de Preguntas - Chavo.xlsx' y
...
XLSX = "Planilla de Preguntas - Chavo 1.xlsx"
MODELO = "MODELO DE INFORME 2.docx"
OUTPUT = "INFORME_Chavo 1.docx"
RADAR_FILE = "grafico_radar_Chavo.png"
...
# Valores evaluado (percentil normalizado 0-1, Laissez invertido)
...
# Perfil ideal líder situacional completo
...
ax.plot(angles, ev, "o-", linewidth=2, color="#FF6600", label="Chavo")
...
ax.set_title("Coherencia Personalidad – Conductas de Liderazgo (Chavo)", y=1.08, fontsize=11)
plt.tight_layout()
...
neo_interp = {
    "N": (
        ...
        neo_niv["N"],
        "Nivel promedio de estabilidad emocional. Puede experimentar estrés ante demandas elevadas, aunque mantiene funcionamiento adecuado en la mayoría de las situaciones. Es un punto a monitorear en roles de conducción bajo presión sostenida."
        if neo_niv["N"] == "Promedio" else
        "Persona emocionalmente estable, resistente al estrés. Fortaleza clave para el rol de conducción."
    ),
    "E": (
        f"PD={neo_raw['E']}  |  T={neo_t['E']}",
        neo_niv["E"],
        "Persona sociable, enérgica y con facilidad para entablar vínculos. Disfruta del trato interpersonal y del trabajo en equipo. Rasgo central para liderar y construir relaciones sólidas con el equipo."
    ),
    "O": (
        f"PD={neo_raw['O']}  |  T={neo_t['O']}",
        neo_niv["O"],
        "Nivel equilibrado de apertura a nuevas ideas. Se adapta a los cambios cuando es necesario, aunque no tiene un perfil marcadamente innovador. Combina pragmatismo y receptividad a la novedad según la situación."
    ),
    "A": (
        f"PD={neo_raw['A']}  |  T={neo_t['A']}",
        neo_niv["A"],
        "Perfil equilibrado entre cooperación y asertividad. Puede ser empático y flexible en el trato, sin perder la capacidad de sostener posiciones cuando es necesario. Compatible con un estilo relacional y colaborativo."
    ),
    "C": (
        f"PD={neo_raw['C']}  |  T={neo_t['C']}",
        neo_niv["C"],
        "Nivel adecuado de organización y orientación al logro. Disciplinado/a cuando la tarea lo requiere, sin rasgos de rigidez. Flexibilidad para adaptarse a imprevistos sin perder el foco en los objetivos."
    ),
}
...
neo_desc = [
    (f"{_neo_dim_nombres[d]} — {neo_niv[d]} (T={neo_t[d]})", _neo_textos[d][neo_niv[d]])
    for d in "NEOAC"
]
...
add_p(f"La persona evaluada muestra un perfil de liderazgo Transformacional predominante ({transf_total:.2f} / P{cel_p['TransfTot']}), con Transaccional complementario ({trans_total:.2f} / P{cel_p['TransTot']}) y una presencia moderadamente alta de Laissez-Faire ({laissez:.2f} / P{cel_p['Laissez']}), que constituye el hallazgo más destacado y el principal foco de desarrollo.")

p = doc.add_paragraph()
bold_run(p, "Liderazgo Transformacional : ")
normal_run(p, f"Alto (P{cel_p['TransfTot']}). Muestra un perfil transformacional con fortalezas en Consideración Individualizada ({cons_ind:.2f} / P{cel_p['ConsInd']}) e Inspiración ({inspir:.2f} / P{cel_p['Inspir']}): atiende el desarrollo de cada colaborador/a y logra comunicar propósitos de manera motivadora. El Carisma ({carisma:.2f} / P{cel_p['Carisma']}) y la Estimulación Intelectual ({est_int:.2f} / P{cel_p['EstimInt']}) en nivel Medio indican que el cuestionamiento analítico y el impacto simbólico son zonas de crecimiento.")

p = doc.add_paragraph()
bold_run(p, "Liderazgo Transaccional : ")
normal_run(p, f"Medio (P{cel_p['TransTot']}). La Dirección por Excepción (P{cel_p['DirExc']}) y la Recompensa Contingente (P{cel_p['RecCont']}) están en niveles moderados. Interviene ante desvíos pero podría fortalecer el reconocimiento sistemático del desempeño esperado como herramienta motivacional cotidiana.")

p = doc.add_paragraph()
bold_run(p, "Laissez-Faire : ")
normal_run(p, f"Moderado-Alto (P{cel_p['Laissez']}). Esta es la zona de mayor atención del perfil. Puede presentar tendencia a la no-intervención o a delegar decisiones sin el acompañamiento necesario. En contextos de equipo maduro puede ser una fortaleza (autonomía), pero ante colaboradores con menor madurez puede generar falta de dirección. Requiere monitoreo activo y desarrollo de mayor presencia directiva cuando la situación lo exige.")
...
normal_run(p, f"Alta Motivación Intrínseca (P{pot_p['Intr']}): lidera porque lo disfruta, por convicción personal y orientación natural al rol. La Motivación Extrínseca (P{pot_p['Extr']}) y Social Normativa (P{pot_p['Soc']}) en nivel Medio indican que el liderazgo no está impulsado principalmente por beneficios materiales ni por obligación social, sino por vocación genuina.")
...
normal_run(p, f"Alto (P{cam_p['Cons']}). Fortaleza destacada: atiende activamente las necesidades personales de los colaboradores, genera un ambiente de bienestar y contención. Complementa muy bien la alta Amabilidad del perfil NEO.")

p = doc.add_paragraph()
bold_run(p, "Conducta Participativa : ")
normal_run(p, f"Alto (P{cam_p['Part']}). Estilo dominante junto con el Considerado: consulta, escucha e involucra activamente al equipo en las decisiones. Genera sentido de pertenencia y compromiso. Es su sello de gestión más distintivo.")

p = doc.add_paragraph()
bold_run(p, "Conducta Directiva : ")
normal_run(p, f"Medio (P{cam_p['Dir']}). Comunica expectativas con claridad en situaciones que lo requieren, aunque no es su estilo predominante. Hay espacio para fortalecer la estructura y la clarificación de roles en momentos de alta demanda o con colaboradores que necesitan más guía.")

p = doc.add_paragraph()
bold_run(p, "Conducta Orientada a Metas : ")
normal_run(p, f"Alto (P{cam_p['Or']}). Establece objetivos exigentes y alienta el rendimiento superior. Esta combinación de alta orientación a metas con alto considerado y participativo es potente: el equipo recibe tanto apoyo como desafío.")

p = doc.add_paragraph()
bold_run(p, "En cuanto a CONLID-A: ")
normal_run(p, f"Distribución con claro predominio en Conductas Orientadas a las Relaciones (P{con_p['Rel']}): reconocimiento de logros, apoyo, contacto cercano, información al grupo. Las Conductas Orientadas al Cambio (P{con_p['Camb']}) también son destacadas: genera alianzas, forma equipos para el cambio y promueve nuevas estrategias. Las Conductas de Tarea (P{con_p['Tar']}) en nivel Medio sugieren que el monitoreo sistemático, la coordinación de actividades y la definición de estándares son áreas de desarrollo para consolidar el rol.")
...
add_p(f"El hallazgo más relevante del perfil es la combinación de un altísimo Liderazgo Considerado (P{cam_p['Cons']}) y Participativo (P{cam_p['Part']}) con una presencia moderadamente alta de Laissez-Faire (P{cel_p['Laissez']}). Esta tensión sugiere que el/la evaluado/a puede alternar entre un acompañamiento muy cercano y episodios de excesiva delegación o no-intervención, especialmente en conflictos o decisiones difíciles. El desarrollo del repertorio directivo y de la intervención activa ante desvíos es la prioridad de desarrollo más urgente.")

doc.add_paragraph()

add_p(f"En términos del modelo Situacional (Hersey & Blanchard) y Camino-Meta (House), el/la evaluado/a maneja con alta solvencia los estilos Considerado y Participativo, y tiene también buena disposición hacia las Metas (P{cam_p['Or']}). El estilo Directivo (P{cam_p['Dir']}) está presente pero es el menos desarrollado. Para completar el repertorio situacional, la clave está en aumentar la presencia del Liderazgo Directivo en situaciones que lo requieran y en reducir los episodios de Laissez-Faire.")
...
recs = [
    ("1. Reducir episodios de Laissez-Faire",
     f"Laissez-Faire P{cel_p['Laissez']}: tendencia a la no-intervención que puede generar ambigüedad de rol en el equipo. Alta Amabilidad (T={neo_t['A']}) puede dificultar la confrontación directa.",
     "Definir criterios claros de cuándo intervenir vs. delegar. Trabajar el liderazgo por excepción activa. Formación en gestión del conflicto y toma de decisiones difíciles. Solicitar feedback al equipo sobre momentos en que se esperaba mayor dirección."),
    ("2. Fortalecer el Liderazgo Directivo",
     f"Directivo P{cam_p['Dir']}: nivel adecuado pero el menos desarrollado del perfil. Necesario para situaciones de baja madurez del colaborador o alta urgencia.",
     "Práctica de comunicación de expectativas claras y no negociables. Role-play de conversaciones directivas. Trabajar con indicadores de desempeño explícitos y dar feedback de corrección de manera oportuna."),
    ("3. Incrementar la Recompensa Contingente",
     f"RecCont P{cel_p['RecCont']}: nivel bajo en el perfil transaccional. Puede generar sensación de que el buen desempeño no es sistemáticamente reconocido.",
     "Implementar un sistema explícito de reconocimiento contingente (formal e informal). Pasar de la Dirección por Excepción como estilo predominante a un balance con el reconocimiento proactivo. Formalizar acuerdos de desempeño + recompensa."),
    ("4. Desarrollar Carisma y Estimulación Intelectual",
     f"Carisma P{cel_p['Carisma']} y EstimInt P{cel_p['EstimInt']}: el componente de influencia simbólica y cuestionamiento analítico están en nivel Medio. La alta Extraversión (T={neo_t['E']}) es el punto de partida.",
     "Entrenamiento en storytelling y construcción de relato de propósito. Incorporar desafíos intelectuales al equipo: debates, análisis de problemas complejos, espacios de innovación. Trabajar el uso del lenguaje inspiracional en comunicaciones formales."),
    ("5. Gestionar la autorregulación emocional",
     f"Neuroticismo en T={neo_t['N']} (Promedio): puede impactar en la gestión de situaciones de presión sostenida. Es la base para sostener el estilo Considerado sin agotamiento.",
     "Técnicas de mindfulness y gestión del estrés. Establecer límites y rutinas de recuperación. Construir red de apoyo entre pares líderes. Supervisión o coaching ejecutivo para procesar la demanda emocional del rol."),
]
...
add_p("El siguiente gráfico contrasta el perfil de Chavo con el perfil ideal de un Líder Situacional. Las zonas donde la línea del evaluado/a (naranja) se acerca al ideal (azul) representan fortalezas consolidadas; las zonas con mayor distancia indican brechas de desarrollo.", italic=True, size=10)
...
add_p(f"Brechas principales: Laissez-Faire elevado (P{cel_p['Laissez']} — invertido en el gráfico), Carisma (P{cel_p['Carisma']} vs. ideal P90), Estimulación Intelectual (P{cel_p['EstimInt']} vs. ideal P85), Recompensa Contingente (P{cel_p['RecCont']} vs. ideal P75).")
add_p(f"Brechas moderadas: Liderazgo Directivo (P{cam_p['Dir']} vs. ideal P75) y Conductas de Tarea (P{con_p['Tar']} vs. ideal P75).")

doc.add_page_break()

# ═══════════════════════════════════════════════════════
# 5. SÍNTESIS DE EVALUACIÓN DE LIDERAZGO
# ═══════════════════════════════════════════════════════
...
fortalezas = []
if cel_p["ConsInd"] >= 75:
    fortalezas.append(f"Consideración Individualizada (P{cel_p['ConsInd']}): atiende activamente el desarrollo y las necesidades de cada colaborador/a, construyendo vínculos de confianza sólidos.")
if cam_p["Cons"] >= 75:
    fortalezas.append(f"Liderazgo Considerado (P{cam_p['Cons']}): genera un ambiente de bienestar, contención y apoyo que favorece la retención y el compromiso del equipo.")
if cam_p["Part"] >= 75:
    fortalezas.append(f"Liderazgo Participativo (P{cam_p['Part']}): involucra y consulta activamente al equipo en las decisiones, generando sentido de pertenencia y apropiación de los objetivos.")
if cam_p["Or"] >= 75:
    fortalezas.append(f"Orientación a Metas (P{cam_p['Or']}): establece objetivos exigentes y alienta el rendimiento superior, combinando desafío con apoyo.")
if con_p["Rel"] >= 75:
    fortalezas.append(f"Conductas de Relaciones (P{con_p['Rel']}): reconocimiento de logros, apoyo cercano e información fluida al equipo como herramientas cotidianas de gestión.")
if con_p["Camb"] >= 75:
    fortalezas.append(f"Conductas Orientadas al Cambio (P{con_p['Camb']}): genera alianzas, promueve nuevas estrategias y forma equipos orientados a la transformación.")
if cel_p["TransfTot"] >= 75:
    fortalezas.append(f"Liderazgo Transformacional (P{cel_p['TransfTot']}): inspira y motiva al equipo hacia metas compartidas, trascendiendo el intercambio puramente transaccional.")
if pot_p["Intr"] >= 75:
    fortalezas.append(f"Motivación Intrínseca (P{pot_p['Intr']}): ejerce el liderazgo por convicción y disfrute genuino del rol, lo que se traduce en consistencia y autenticidad.")
if neo_niv["E"] in ("Alto", "Muy Alto"):
    fortalezas.append(f"Extraversión ({neo_niv['E']}, T={neo_t['E']}): sociabilidad y energía natural para construir vínculos de confianza y mantener al equipo conectado.")
if neo_niv["N"] in ("Bajo", "Muy Bajo"):
    fortalezas.append(f"Estabilidad Emocional ({neo_niv['N']}, T={neo_t['N']}): manejo sólido del estrés y la presión del rol, recurso fundamental para el liderazgo sostenido.")
if not fortalezas:
    fortalezas.append("El perfil no presenta dimensiones con puntajes destacados por encima del percentil 75. Ver análisis detallado en secciones anteriores.")
...
areas = []
if cel_p["Laissez"] >= 75:
    areas.append(f"Tendencia Laissez-Faire (P{cel_p['Laissez']}): reducir los episodios de no-intervención o delegación sin acompañamiento, especialmente con colaboradores de menor madurez o en situaciones de conflicto.")
if cam_p["Dir"] < 50:
    areas.append(f"Liderazgo Directivo (P{cam_p['Dir']}): fortalecer la capacidad de dar instrucciones claras, establecer expectativas no negociables y estructurar el trabajo en situaciones de urgencia o con colaboradores que requieren mayor guía.")
if cel_p["RecCont"] < 50:
    areas.append(f"Recompensa Contingente (P{cel_p['RecCont']}): implementar un sistema explícito y sistemático de reconocimiento del buen desempeño como herramienta de motivación y refuerzo cotidiano.")
if cel_p["Carisma"] < 50:
    areas.append(f"Carisma e Influencia Simbólica (P{cel_p['Carisma']}): desarrollar el impacto simbólico, el uso del relato y la capacidad de inspirar a través de la presencia y la comunicación.")
if cel_p["EstimInt"] < 50:
    areas.append(f"Estimulación Intelectual (P{cel_p['EstimInt']}): incorporar el cuestionamiento analítico, el debate y el desafío intelectual como herramientas activas de desarrollo del equipo.")
if con_p["Tar"] < 50:
    areas.append(f"Conductas de Tarea (P{con_p['Tar']}): fortalecer el monitoreo sistemático, la coordinación de actividades y la definición explícita de estándares de desempeño.")
if neo_niv["N"] in ("Alto", "Muy Alto"):
    areas.append(f"Autorregulación Emocional (Neuroticismo {neo_niv['N']}, T={neo_t['N']}): desarrollar estrategias activas para gestionar la reactividad emocional y sostener el rol bajo presión sostenida sin agotamiento.")
if not areas:
    areas.append("El perfil no presenta brechas significativas en las dimensiones evaluadas. Ver análisis detallado en secciones anteriores.")
...
# ── Objetivos de Desarrollo Sugeridos ──
add_p("Objetivos de Desarrollo Sugeridos", bold=True, size=11)

objetivos = []
if cel_p["Laissez"] >= 75 or cam_p["Dir"] < 50:
    objetivos.append("Ampliar el repertorio directivo: practicar la intervención activa ante desvíos y la comunicación de expectativas no negociables. Definir criterios explícitos de cuándo dirigir, cuándo acompañar y cuándo delegar, según la madurez del colaborador y la urgencia de la situación.")
if cel_p["RecCont"] < 50 or cel_p["DirExc"] < 50:
    objetivos.append("Implementar un sistema de reconocimiento contingente: formalizar acuerdos de desempeño con recompensas asociadas y pasar de un reconocimiento espontáneo a uno sistemático, explícito y oportuno.")
if cel_p["Carisma"] < 50 or cel_p["EstimInt"] < 50:
    objetivos.append("Desarrollar el impacto transformacional: entrenamiento en storytelling, construcción de relato de propósito compartido e incorporación de espacios de innovación y desafío intelectual en la dinámica del equipo.")
if neo_niv["N"] not in ("Bajo", "Muy Bajo"):
    objetivos.append("Fortalecer la autorregulación emocional: desarrollar técnicas de gestión del estrés (mindfulness, supervisión entre pares), establecer rutinas de recuperación y construir una red de apoyo entre líderes del mismo nivel.")
if con_p["Tar"] < 50:
    objetivos.append("Consolidar las conductas de tarea: establecer rutinas de monitoreo de indicadores clave, definir estándares explícitos de desempeño y practicar el feedback de corrección de manera sistemática y oportuna.")
if not objetivos:
    objetivos.append("Continuar profundizando las fortalezas identificadas a través de procesos de coaching o mentoring ejecutivo que potencien el impacto ya consolidado.")
    objetivos.append("Solicitar feedback periódico del equipo y de pares sobre aspectos concretos de gestión para sostener el desarrollo continuo basado en evidencia.")
